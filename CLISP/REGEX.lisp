...
(defun make-new-deltas (auto) (MND auto (more2 (get-deltas auto)))
(defun MND (auto m2)
    (cond
        ((null m2) nil)
        (T (append (make-fnodes (car m2) auto (cddar m2)) (MND auto (cdr m2))))
    )
)
...
(defun del-eps(auto)
    (cond
        ((null auto) nil)
        ((eq (cadar (cdr auto)) 'eps) (cons (makeauto (caar auto) (cadar auto) (caddr (cadr auto))) (del-eps (cddr auto))))
        (T (append (list (car auto)) (del-eps (cdr auto))))
    )
)
;-----------------------------------------------------------main------------------------------------------------------------------
...
(print '=>)
(print (make-new-deltas (del-eps (regex '(#\a + #\b \| #\a #\b * #\a)))))
; (print (get-deltas (del-eps (regex '(#\a + #\b \| #\a #\b * #\a)))))
; (terpri)
; (print (regex '(#\a #\b + #\a * #\b \| #\a #\b + #\a + #\b + #\b)))
; (print '=>)
; (print (determinate (regex '(#\a #\b + #\a * #\b \| #\a #\b + #\a + #\b + #\b))))
; (print (regex '(#\a #\b * #\b \| #\a #\b)))
; (print '=>)
; (print (determinate (regex '(#\a #\b * #\b \| #\a #\b))))
