...
(defun search-byp (delta auto)
    (cond
        ((null auto) nil)
        ((eq delta (caar auto)) T)
        (T (search-by delta (cdr auto)))
    )
)
(defun search-by (delta auto)
    (cond
        ((null auto) nil)
...
(defun make-nodes (delta auto node)
    (cond
        ((null auto) nil)
        ((search-byp (car delta) auto) (append (list (append (list node) (search-by (car delta) auto)))(make-nodes (cdr delta) (cdr auto) node)))
        (T (make-nodes (cdr delta) (cdr auto) node))
    )
)
(defun make-new-deltas (auto) (MND auto (more2 (get-deltas auto)) 1000))
(defun MND (auto m2 node)
    (cond
        ((null m2) nil)
        (T (append (make-fnodes (car m2) auto node) (MND auto (cdr m2) (+ 1000 node))))
    )
)
(defun determinate (automaton)
    (append automaton (make-new-deltas automaton))
)
;-----------------------------------------------------------main------------------------------------------------------------------
